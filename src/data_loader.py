...
class TranslationDataset(Dataset):
    ...
    # def _extract_translation_pairs(self, data: Dict[str, Any]) -> List[Tuple[str, str]]:
    #     """从三层嵌套字典结构中提取翻译对"""
    #     pairs = []
    #
    #     # 遍历所有源语言
    #     for src_lang, target_langs in data.items():
    #         if not isinstance(target_langs, dict):
    #             continue
    #
    #         # 遍历所有目标语言
    ...
    #
    #             # 检查是否匹配我们需要的语言对
    #             if src_lang == self.source_lang and tgt_lang == self.target_lang:
    #                 logging.info(f"找到 {src_lang}->{tgt_lang} 翻译对，包含 {len(translations)} 个样本")
    #
    ...
    #                             if len(pairs) <= 3:  # 显示前3个样本
    #                                 logging.info(f"  样本 {key}:")
    #                                 logging.info(f"    Source: {source_text[:100]}...")
    #                                 logging.info(f"    Translated: {translated_text[:100]}...")
    #
    #     # 如果没有找到，尝试反向查找
    #     if not pairs:
    #         logging.info(
    #             f"未找到 {self.source_lang}->{self.target_lang}，尝试反向查找 {self.target_lang}->{self.source_lang}")
    #         for src_lang, target_langs in data.items():
    #             if not isinstance(target_langs, dict):
    #                 continue
    #
    ...
    #                     continue
    #
    #                 if src_lang == self.target_lang and tgt_lang == self.source_lang:
    #                     logging.info(f"找到反向 {src_lang}->{tgt_lang} 翻译对，包含 {len(translations)} 个样本")
    #
    #                     for key, translation_data in translations.items():
    #                         if isinstance(translation_data, dict):
    #                             source_text = translation_data.get('Source', '')
    ...
    #
    #                             if source_text and translated_text:
    #                                 # 交换方向：目标语言作为源文本，源语言作为目标文本
    #                                 pairs.append((translated_text, source_text))
    #                                 if len(pairs) <= 3:
    #                                     logging.info(f"  样本 {key} (反向):")
    #                                     logging.info(f"    Source: {translated_text[:100]}...")
    #                                     logging.info(f"    Translated: {source_text[:100]}...")
    #
    #     # 如果还是没有找到，使用英语到任意语言的翻译作为训练数据
    #     if not pairs:
    #         logging.info("使用英语到其他语言的翻译作为训练数据")
    #         for src_lang, target_langs in data.items():
    #             if src_lang == "en":  # 英语作为源语言
    #                 for tgt_lang, translations in target_langs.items():
    #                     if isinstance(translations, dict):
    #                         logging.info(f"使用 en->{tgt_lang} 的 {len(translations)} 个样本")
    #
    #                         for key, translation_data in translations.items():
    #                             if isinstance(translation_data, dict):
    #                                 source_text = translation_data.get('Source', '')
...
class DataProcessor:
    ...
    def extract_all_texts(self, data: Dict[str, Any]) -> Tuple[List[str], List[str]]:
        ...
        # # 方法2: 查找反向 de->en
        # if 'de' in data and isinstance(data['de'], dict) and 'en' in data['de']:
        #     translations = data['de']['en']
        #     # if isinstance(translations, dict):
        #     logging.info(f"找到 de->en (反向使用): {len(translations)} 个样本")
        #     for translation_data in translations:
        #         if isinstance(translation_data, dict):
        #             source_text = translation_data.get('Source', '')
        #             translated_text = translation_data.get('Translated', '')
        ...
        #                 tgt_texts.append(source_text)

        # # 方法3: 使用英语到其他语言的翻译
        # else:
        #     logging.info("使用英语到其他语言的翻译")
        #     if 'en' in data and isinstance(data['en'], dict):
        #         for tgt_lang, translations in data['en'].items():
        #             if isinstance(translations, dict):
        #                 logging.info(f"使用 en->{tgt_lang}: {len(translations)} 个样本")
        #                 for key, translation_data in translations.items():
        #                     if isinstance(translation_data, dict):
        #                         source_text = translation_data.get('Source', '')
        #                         translated_text = translation_data.get('Translated', '')
        ...
        #                             src_texts.append(source_text)
        #                             tgt_texts.append(translated_text)
        #                             if len(src_texts) >= 5000:  # 限制数量
        #                                 break
        #                 if len(src_texts) >= 5000:
        #                     break
